...
class DbCommentRepository {
    ...
    async save(comment) {
        ...
        try {
            ...
            console.error(`[CommentRepo ERROR] Failed to fetch user name with query: "${userQuery}"`, error);
        }
        const query = `
      ...
      `;
        try {
            ...
            console.error("❌ [CommentRepo FINAL INSERT ERROR] 댓글 삽입 실패:", insertError);
    ...
    async findAll(article_id, offset, limit) {
        ...
        console.log(`[findAll Debug] 전달된 articleId: ${articleId}, Type: ${typeof articleId}`);
